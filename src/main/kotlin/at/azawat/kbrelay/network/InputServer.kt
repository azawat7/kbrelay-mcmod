                    ...
                    try {
                        ...
                        while (!server.isClosed) {
                            try {
                                ...
                                KbRelayMod.logger.info(
                                    "KBRelay: C# app connected from ${client.remoteSocketAddress}"
                                )
                                handleClient(client)
                    ...
                    try {
                        socket.use { sock ->
                            ...
                            while (reader.readLine().also { line = it } != null) {
                                ...
                                when (msg.type) {
                                    "key_down",
                                    "key_up" -> handler.enqueue(msg)
                                    else ->
                                        KbRelayMod.logger.warn(
                                            "KBRelay: unknown message type '${msg.type}'"
                                        )
                                }
                            }
                            KbRelayMod.logger.info(
                                "KBRelay: readLine() returned null (client closed write end)"
                            )
                        }
                    } catch (e: Exception) {
                        KbRelayMod.logger.warn(
                            "KBRelay: connection error: ${e::class.simpleName}: ${e.message}"
                        )
                    } finally {
                        KbRelayMod.logger.info(
                            "KBRelay: client handler exiting, releasing all keys"
                        )
                        handler.releaseAll()
                    }
                },
